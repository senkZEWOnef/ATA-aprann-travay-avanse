...
import { pythonMidtermExam, type ExamQuestion } from '@/data/pythonMidtermExam';
...
interface PythonMidtermExamProps {
  onComplete?: (score: number, totalPoints: number, timeSpent: number, answers: any) => void;
}

export default function PythonMidtermExam({ onComplete }: PythonMidtermExamProps) {
  const locale = useLocale();
  const [hasStarted, setHasStarted] = useState(false);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, any>>({});
  const [timeLeft, setTimeLeft] = useState(pythonMidtermExam.duration * 60); // Convert to seconds
  ...
  const currentQuestion = pythonMidtermExam.questions[currentQuestionIndex];
  const isLastQuestion = currentQuestionIndex === pythonMidtermExam.questions.length - 1;
  ...
  const calculateScore = () => {
    ...
    pythonMidtermExam.questions.forEach(question => {
      ...
      if (question.type === 'multiple-choice') {
        ...
        if (userAnswer && userAnswer.trim().length > 10) {
          ...
          if (question.id === 'code-001') {
            ...
            requiredElements = 3;
          } else if (question.id === 'code-002') {
            if (code.includes('[1, 2, 3, 4, 5]')) hasKeyElements++;
            if (code.includes('.append(6)')) hasKeyElements++;
  ...
  const handleSubmit = () => {
    ...
    const timeSpent = Math.round((pythonMidtermExam.duration * 60 - timeLeft) / 60); // Convert to minutes
  ...
  const nextQuestion = () => {
    if (currentQuestionIndex < pythonMidtermExam.questions.length - 1) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">
                {getLocalizedText(pythonMidtermExam.title)}
              </h1>
              <p className="text-red-100 mt-2">
                {getLocalizedText(pythonMidtermExam.description)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          <div className="mb-8">
            ...
            <ul className="space-y-2">
              {getLocalizedText(pythonMidtermExam.instructions).map((instruction, index) => (
          ...
          <div className="bg-gray-50 rounded-xl p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                ...
                <div className="text-lg font-semibold text-gray-900">{pythonMidtermExam.duration} {locale === 'ht' ? 'minit' : locale === 'fr' ? 'minutes' : 'minutes'}</div>
              ...
              <div className="text-center">
                ...
                <div className="text-lg font-semibold text-gray-900">{pythonMidtermExam.questions.length} {locale === 'ht' ? 'kesyon' : locale === 'fr' ? 'questions' : 'questions'}</div>
                <div className="text-sm text-gray-600">
                  {locale === 'ht' ? '20 multiple choice + 10 kod' : locale === 'fr' ? '20 choix multiple + 10 code' : '20 multiple choice + 10 code'}
                </div>
              </div>
              <div className="text-center">
                <AcademicCapIcon className="w-8 h-8 text-green-500 mx-auto mb-2" />
                <div className="text-lg font-semibold text-gray-900">{pythonMidtermExam.totalPoints} {locale === 'ht' ? 'pwen' : locale === 'fr' ? 'points' : 'points'}</div>
                <div className="text-sm text-gray-600">
  ...
  if (showResults) {
    ...
    const passed = percentage >= pythonMidtermExam.passingScore;
          ...
          <div className="text-center">
            ...
            <h1 className="text-3xl font-bold mb-2">
              {passed 
                ...
                : (locale === 'ht' ? 'Egzamen Fini' : locale === 'fr' ? 'Examen Terminé' : 'Exam Complete')
              }
            </h1>
            <p className="text-lg opacity-90">
              {locale === 'ht' ? 'Ou fini egzamen midterm Python an' : 
               locale === 'fr' ? 'Vous avez terminé l\'examen de mi-session Python' :
               'You have completed the Python midterm exam'}
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-lg font-semibold">
                  {pythonMidtermExam.questions.filter(q => q.type === 'multiple-choice' && answers[q.id] === q.correctAnswer).length} / {pythonMidtermExam.questions.filter(q => q.type === 'multiple-choice').length}
                </div>
              </div>
              <div>
                <div className="text-sm text-gray-600">
                ...
                <div className="text-lg font-semibold">
                  {pythonMidtermExam.questions.filter(q => q.type === 'code' && answers[q.id]).length} / {pythonMidtermExam.questions.filter(q => q.type === 'code').length}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="text-gray-600 mb-4">
              {passed 
                ? (locale === 'ht' ? 'Ou gen dwa kontinye ak leson yo ki vin apre yo' : 
                   locale === 'fr' ? 'Vous pouvez continuer avec les leçons suivantes' :
                   'You may continue with the next lessons')
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-4 mt-2 text-red-100">
              <span>
                {locale === 'ht' ? 'Kesyon' : locale === 'fr' ? 'Question' : 'Question'} {currentQuestionIndex + 1} / {pythonMidtermExam.questions.length}
              </span>
              <span>•</span>
        ...
        {/* Progress bar */}
        <div className="mt-4">
          <div className="w-full bg-red-800 bg-opacity-30 rounded-full h-2">
            <div 
              className="bg-white h-2 rounded-full transition-all duration-300"
              style={{ width: `${((currentQuestionIndex + 1) / pythonMidtermExam.questions.length) * 100}%` }}
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex">
        ...
        <div className="w-64 bg-gray-50 p-4">
          ...
          <div className="space-y-1">
            {pythonMidtermExam.questions.map((q, index) => (
              <button
                key={q.id}
